...
const locales = import.meta.glob('../../../public/console/translations/*.json')
...
export class Presentation {
	...
	async init({
		...
		// FETCHING DICTIONARY...
		const prefix = '../../../public/console/translations/'
		if (!locales[prefix + this.languageCode + '.json'])
			this.languageCode = 'en'

		const dictionary = await locales[prefix + this.languageCode + '.json']()
		...
		print(reelsToString({reels: initialReels}) + '\n'
			...
			+ dictionary.available_bets + ': ' + betsOptions.map(
	...
	async getUserInput({
		...
		const betIndex = this.betsOptions.indexOf(bet)
		const { dictionary } = this
		const optionsMap = {
			1: { key: 'make_bet' },
			2: {
				key: 'change_bet',
				value: (betIndex + 1) % this.betsOptions.length
			},
			3: {
				key: 'buy_bonus',
				value: betIndex
		...
		print(
			...
			+ formatMoney(this.buyFeatureOptions[betIndex], currencyCode) + '\n'
			+ '---------' + '\n'
